...
export default function Admin() {
  // auth / UI
  const [authorized, setAuthorized] = useState(true) // liberado por simplicidade
  const [checked, setChecked] = useState(true)
  const [viewMode, setViewMode] = useState('full')

  // data states (principalmente vindo do Supabase)
  ...
  // Fetch helpers (fetch from supabase; fallback to localStorage)
  ...
  const deleteRestricao = async (id) => {
    try {
      ...
      console.error('deleteRestricao err - fallback local', err)
  ...
  // Substitua sua função uploadAlbumFiles por esta para depuração
const uploadAlbumFiles = async (files) => {
  console.log('uploadAlbumFiles called', files)
  const resultRows = []
  for (const file of files) {
    try {
      ...
      const { data: uploadData, error: uploadError } = await supabase.storage.from('album_ibr').upload(fileName, file, {
        cacheControl: '3600',
        upsert: false
      })
      ...
      if (uploadError) {
        // mostra erro detalhado ao usuário e continua para próximos arquivos
        console.error('Upload error for', file.name, uploadError)
        alert(`Erro ao enviar ${file.name}: ${uploadError.message || JSON.stringify(uploadError)}`)
        continue
      }

      // pegar public url (verifique o campo retornado)
      const { data: publicData, error: publicError } = await supabase.storage.from('album_ibr').getPublicUrl(fileName)
      console.log('publicData:', publicData, 'publicError:', publicError)
      if (publicError) {
        console.error('getPublicUrl error', publicError)
      }
      const publicUrl = publicData?.publicUrl || null
      if (!publicUrl) {
        console.warn('Public URL não retornada. Verifique se o bucket é público.')
      }

      // inserir registro na tabela album
      const { data: row, error: insertErr } = await supabase.from('album').insert([{ title: file.name, image_url: publicUrl }]).select().single()
      console.log('insert row:', row, 'insertErr:', insertErr)
      if (insertErr) {
        console.error('Erro ao inserir row em album', insertErr)
        alert(`Erro ao criar registro no DB para ${file.name}: ${insertErr.message || JSON.stringify(insertErr)}`)
        continue
      }

      resultRows.push(row)
    } catch (err) {
      console.error('uploadAlbumFiles inner err', err)
  ...
  const handleAddMemberClick = async () => {
    ...
    if (!nome) return
    const payload = { nome, funcao: funcoesSelecionadas }
  ...
  const handleAddInformativo = async () => {
    ...
    if (!titulo) return
    // If user attached an imageDataUrl, we can attempt to upload it to storage first.
    // For simplicity, if informativoImagem is a dataURL, we keep it as imagem_url string in DB (not ideal for large images).
    // Preferably upload to storage as real file — but that requires reconstructing Blob from dataURL.
    let imagem_url = null
    if (informativoImagem && typeof informativoImagem === 'string' && informativoImagem.startsWith('data:')) {
      ...
      try {
        ...
        const { data: uploadData, error: uploadError } = await supabase.storage.from('album_ibr').upload(fileName, blob)
        if (!uploadError) {
          const { data: publicData } = await supabase.storage.from('album_ibr').getPublicUrl(fileName)
          imagem_url = publicData?.publicUrl || null
        } else {
          console.warn('informativo image upload error', uploadError)
        }
      } catch (err) {
        console.error('informativo image upload convert err', err)
      }
    }
    const payload = { titulo, conteudo: informativoConteudo.trim(), imagem_url }
    await addInformativo(payload)
  ...
  const handleSaveEditInformativo = async () => {
    ...
    const id = editingInformativo.id
    const payload = { titulo: editingInformativo.titulo, conteudo: editingInformativo.conteudo, imagem_url: editingInformativo.imagemDataUrl || editingInformativo.imagem_url || null }
    await updateInformativo(id, payload)
        ...
        funcoesEscala.forEach(f => {
          const candidatos = membros.filter(m => {
            ...
            const temR = restricoes.some(r => normalizeDateStr(r.data) === dataNorm &&
              ...
              (r.member_id === m.id || namesMatch(m.nome, r.responsavel)))
            return !temR
          })
          const escolhido = candidatos.length ? candidatos[Math.floor(Math.random() * candidatos.length)] : null
          if (escolhido) { alocados[f] = escolhido.nome; assignedKeys.add(escolhido.id) } else { alocados[f] = 'Vago' }
        })
        turnos[p] = alocados
      })
  ...
  const handlePublishEscala = async (escalaArr) => {
    ...
    const obj = { monthLabel, data: escalaArr }
    await publishEscalaMensal(obj)
  ...
  useEffect(() => {
    // initial fetch
    fetchAll()
    // optional: you can add realtime subscriptions here later
    // cleanup: none for now
  }, [fetchAll])
            ...
            <div style={{ padding: 20, textAlign: 'center', background: '#f9f9f9', borderRadius: 10, border: '2px dashed #ddd' }}>
              ...
              <div style={{ marginTop: 25, borderTop: '1px solid #eee', paddingTop: 15 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: 10 }}>
                  {album.length === 0 ? <p style={{ color: '#999', gridColumn: '1/-1' }}>Nenhuma foto ainda.</p> : album.map(foto => (
              ...
              <div style={{ marginTop: 12 }}>
                {sugestaoEditada.map(dia => (
                  <div key={dia.data} style={{ padding: 12, background: '#fff', borderRadius: 6, border: '1px solid #f3f3f3', marginBottom: 10 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <strong style={{ display: 'block', marginBottom: 8 }}>{new Date(dia.data + 'T00:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: 'long' })}</strong>
                      <div><button onClick={() => setEditingDay(dia)} style={{ padding: '6px 10px', borderRadius: 8, border: '1px solid #ddd' }}>Editar</button></div>
                    </div>
                            ...
                            return (
                              <div key={f} style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 8 }}>
                                ...
                                <select value={n} onChange={e => {
                                  const novo = sugestaoEditada.map(sd => sd.data === dia.data ? { ...sd, turnos: { ...sd.turnos, [p]: { ...sd.turnos[p], [f]: e.target.value } } } : sd)
                                ...
                                <button onClick={() => {
                                  ...
                                  const novo = sugestaoEditada.map(sd => sd.data === dia.data ? novoDia : sd)
                                  setSugestaoEditada(novo)
            ...
            <div>
              ...
              {restricoes.length === 0 ? <p style={{ color: '#9ca3af' }}>Nenhuma restrição.</p> : restricoes.map(r => (
                <div key={r.id} style={{ padding: '10px 0', borderBottom: '1px solid #f0f0f0', fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <div><button onClick={() => deleteRestricao(r.id)} style={{ background: '#fee2e2', border: '1px solid #fca5a5', padding: '6px 10px', borderRadius: 8 }}>Remover</button></div>
                </div>
              ))}
            </div>
          )}
